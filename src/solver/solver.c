...
int solve(char * inputFile){
    ...
    while(fgets(line,15,fpointer)){
        if(i!=3 && i!=7){
            for(int j=0;j<3;j++){
                if(line[j]>='0' && line[j]<='9')grid[i-delta][j] = line[j]-48;
            }
            for(int j=4;j<7;j++){
                if(line[j]>='0' && line[j]<='9')grid[i-delta][j-1] = line[j]-48;
            }
            for(int j=8;j<11;j++){
                if(line[j]>='0' && line[j]<='9')grid[i-delta][j-2] = line[j]-48;
            }
        }
        else if(i==3) delta=1;
        else if(i==7) delta=2;
